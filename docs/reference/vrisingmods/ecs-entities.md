...
**Cache your EntityQuery**: `CreateEntityQuery` is not free. Store the resulting `EntityQuery` as a field on your service class and reuse it across calls. Don't recreate it every frame. Do **not** dispose the query itself; only dispose the `NativeArray<Entity>` that `ToEntityArray` returns.

The [Entity Query Descriptions](/dev/query-descriptions.html) reference lists every query registered by a game system, including the field name and component filters. Useful when you need to know which query to access in a Harmony patch.
