...
export const MapCanvas: React.FC<MapCanvasProps> = ({
  ...
  return (
    <div className="relative w-full h-full bg-[#070a11] overflow-hidden select-none flex-1 flex flex-col">
          ...
          >
            ...
            {Object.values(REGIONS).map(r => (
              <text 
                ...
                letterSpacing="3"
                textAnchor="middle"
                className="pointer-events-none uppercase font-mono select-none"
              >
                {r.name}
              </text>
            ))}
      ...
      <div className="absolute bottom-4 right-4 z-20 hidden sm:flex items-center gap-3 bg-slate-950/90 border border-slate-800/80 px-3.5 py-1.5 rounded-xl shadow-2xl text-xs font-mono text-slate-400 backdrop-blur-md">
